...
object PuzzleProfileEngine {
    ...
    private const val SECONDS_PER_LINE_FAST = 4
    private const val SECONDS_PER_LINE_SLOW = 12

    fun updateMetrics(current: PuzzleProfileMetrics, game: NurseryRhymesGame): PuzzleProfileMetrics {
        if (game.status != GameStatus.COMPLETED || game.level.isTutorial) return current

        val lineCount = max(game.level.totalLines, 1)
        val optimalMoves = NurseryRhymesEngine.optimalMoveCount(game)
        val secondsPerLine = game.elapsedSeconds.toFloat() / lineCount
        val isFast = secondsPerLine <= SECONDS_PER_LINE_FAST
        val isSlow = secondsPerLine >= SECONDS_PER_LINE_SLOW
        val isPerfect = game.hintsUsed == 0 && game.moves <= optimalMoves
        val isComplex = game.level.totalLines >= 8 ||
            game.level.difficulty.ordinal >= Difficulty.HARD.ordinal
        val isInefficient = game.moves > (optimalMoves * 1.35f).roundToInt()
    ...
    fun adaptiveGenerationProfile(profile: PuzzleProfile): GenerationProfile {
        ...
        return GenerationProfile(rhymeSpreadModifier = modifier)
    }
